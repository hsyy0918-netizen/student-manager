...
public class StudentServiceImpl implements IStudentService {
    ...
    public List<Course> getAllAvailableCourses(int studentId) {
        ...
        // 获取该学生已选的课程
        List<CourseWithSCourse> selectedCourses = selectedCourseDao.selectStudentSelectedCourses(studentId);
        ...
        if (selectedCourses != null && !selectedCourses.isEmpty()) {
            for (CourseWithSCourse sc : selectedCourses) {
                // 使用流式操作进行过滤，更高效
    ...
    @Override
    public List<CourseWithSCourse> getSelectedCourses(int studentId) {
        SqlSession session = getSession();
        selectedCourseDao = session.getMapper(ISelectedCourseDao.class);
    ...
    @Override
    public List<CourseWithSCourse> getFinishedCourses(int studentId) {
        SqlSession session = getSession();
        selectedCourseDao = session.getMapper(ISelectedCourseDao.class);
